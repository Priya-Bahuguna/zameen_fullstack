// Seed script that can be called as a module (does NOT call process.exit)
...
const SELLERS = [
  ...
  { name:'Priya Singh', phone:'+91-9765432109' },
  { name:'Amit Patel', phone:'+91-9654321098' },
];
...
async function runSeed() {
  ...
  for (const [city, data] of Object.entries(CITIES_DATA)) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let price;
      if (listingType === 'Buy') {
        price = Math.round(data.basePriceBuy * mult * area * furnMult * ageMult * noise);
      } else {
      ...
      properties.push({
        ...
        description: `Beautiful ${bhk}BHK ${propType.toLowerCase()} in the heart of ${locality}, ${city}. ${furn} property.`,
        city, locality, address: `${rnd(1,999)}, ${locality}, ${city}`,
        ...
        amenities: pickN(AMENITIES, rnd(3,9)),
        images: [pick(IMAGES), pick(IMAGES)],
        verified: Math.random() < 0.6, hot: Math.random() < 0.2, featured: Math.random() < 0.1,
        sellerName: seller.name, sellerPhone: seller.phone,
        priceHistory, views: rnd(10,500), inquiries: rnd(0,50), status: 'active',
      });
    }
    ...
    for (const locality of data.localities) {
      ...
      const ratings = { connectivity: r(base), safety: r(base-0.2), schools: r(base-0.3), hospitals: r(base-0.1), markets: r(base+0.1), greenery: r(3.0), nightlife: r(base-0.5), publicTransport: r(base) };
      const overall = Math.round(Object.values(ratings).reduce((a,b)=>a+b,0)/8*10)/10;
      const investmentScore = Math.min(100, Math.round((mult*40 + Math.random()*20 + overall*8)*10)/10);
      areaRatings.push({
        city, locality, overallRating: overall, ratings,
        avgPriceBuy: Math.round(data.basePriceBuy*mult*1000),
        avgPriceRent: Math.round(data.baseRent*mult),
        priceGrowth1Y: Math.round((mult*8+Math.random()*5-2)*10)/10,
        investmentScore,
  ...
  return { properties: properties.length, areaRatings: areaRatings.length };
}
